...
def load_from_gcs(
    ...
    try:
        ...
        https_base_url: str = _gcs_to_https_url(bucket)
        ...
        # Create schema, file format, and internal stage
        _create_schema(session, destination_schema)
        _create_parquet_file_format(session, destination_schema)
        _create_internal_stage(session, destination_schema, DEFAULT_STAGE_NAME)

        # Upload files to internal stage
        _upload_files_to_stage(
            session, local_files, destination_schema, DEFAULT_STAGE_NAME
        )
...
def load_from_s3(
    ...
    try:
        ...
        results: dict[str, int] = _load_all_tables(session, destination_schema)
    except Exception as e:
        raise DataIngestionError(
            f"Failed to load Jaffle Shop data into Snowflake. Error: {e}"
        ) from e
...
def _upload_files_to_stage(
    ...
    for local_path in local_files.values():
        ...
        # PUT file to stage - Snowpark handles the upload
...
def _load_single_table(
    ...
    _ = session.sql(copy_sql).collect()
